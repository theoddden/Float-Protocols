...
impl ClockReconciler {
    pub fn new(max_offsets: usize, max_age: Duration, network_time_source: NetworkTimeSource) -> Self {
        Self {
    ...
    pub fn calculate_offset(device_id: u64, device_timestamp_ms: u64, network_timestamp_ms: u64) -> i64 {
    ...
    pub fn stats(&self) -> ClockStats {
        ...
        let stale_offsets = self.offsets.iter().filter(|o| o.is_stale(self.max_age)).count();
...
mod tests {
    ...
    fn test_clock_reconciliation() {
        let reconciler = ClockReconciler::new(10, Duration::from_secs(300), NetworkTimeSource::LocalClock);
