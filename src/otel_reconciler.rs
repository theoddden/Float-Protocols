...
impl BitemporalReconciler {
    ...
    pub fn ingest(&mut self, span: OtlpSpan) {
        ...
        self.records.entry(entity_id).or_insert_with(Vec::new).push(record);
    }
    ...
    pub fn entity_history(&self, trace_id: &str, span_id: &str) -> Vec<&BitemporalRecord> {
        ...
        self.records.get(&entity_id).map(|v| v.iter().collect()).unwrap_or_default()
    }
...
mod tests {
    ...
    fn test_spread_stats() {
        ...
        for i in 0..10 {
            let compact = CompactSpan::new([i as u8; 16], [i as u8; 8], "sensor.read", "sensor-001")
                .with_t_event(i * 100)
